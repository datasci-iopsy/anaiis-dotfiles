...
- `Agent` (subagent_type=general-purpose, description="Fix CR-<N>: <summary>") for surgical fixes, reusing `code-surgeon` persona from `claude/agents/code-surgeon.md`
- `Bash(uv:*)`, `Bash(Rscript:*)`, `Bash(bun:*)`, `Bash(npm:*)` for test verification

All git commands must start with `git` to match pre-approved patterns. Determine repo root once with `git rev-parse --show-toplevel` and use `-C <root>` throughout.
...
```bash
DETECT=~/.dotfiles/claude/skills/anaiis-coderabbit/lib/detect-tests.sh
if [ -x "$DETECT" ]; then
    bash "$DETECT"
else
    echo "none"
...
This script prints the test command(s) for the project, one per line, or `none` if no test suite is detected. If the script is absent (non-dotfiles environment), the fallback `none` skips testing without failing. Run each command. If any command exits non-zero:
...
- Cluster only if multiple findings touched the same file with the same logical intent (e.g., two NA-guard fixes in the same function).
...
- Never amend commits.
- Never auto-chain into `/anaiis-gitrebase`, `/anaiis-changelog`, or `/anaiis-gitpr`. Skill exits and leaves those to the user.
- Never remove the JSONL run ledger during the session. It is audit trail.
- Never run `coderabbit review` a third time in the same session (two rounds maximum: initial + recheck).

## Integration with other skills

- `/anaiis-gitrebase`: run after this skill to consolidate the CR fix commits into logical groups.
- `/anaiis-changelog`: run after rebase to generate a PR description.
- `/anaiis-gitpr`: run after changelog to open the PR.
- `code-surgeon`: this skill dispatches it via Agent for every rated-3-to-5 finding. No manual invocation needed.

...
- macOS, Bash shell. Shell config is user-managed and not tracked in dotfiles. Machine-local vars (GCP project, API keys, aliases) go in `~/.bashrc.local`, which is not tracked in git. `web-verify` is available via `export PATH="$HOME/.dotfiles/bin:$PATH"` in shell config. Never modify `.zshrc`.
